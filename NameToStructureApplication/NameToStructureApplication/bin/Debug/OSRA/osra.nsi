...
; The file to write
OutFile "osra-setup-1-3-6.exe"

; The default installation directory
InstallDir $PROGRAMFILES\osra\1.3.6

; Registry key to check for directory (so if you install again, it will 
; overwrite the old one automatically)
InstallDirRegKey HKLM "Software\osra\1.3.6" "Install_Dir"
...
Section "osra (required)"
  ...
  WriteRegStr HKLM SOFTWARE\osra\1.3.6 "Install_Dir" "$INSTDIR"
  WriteRegStr HKLM SOFTWARE\osra "Install_Dir" "$INSTDIR"
  
  ; Write the uninstall keys for Windows
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\osra" "DisplayName" "OSRA 1.3.6"
...
Section "Uninstall"
  ReadRegStr $0 HKLM SOFTWARE\osra\1.3.6 "Install_Dir"
  strcpy $INSTDIR $0
  ; Remove registry keys
  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\osra"
  DeleteRegKey HKLM SOFTWARE\osra\1.3.6
