...
using NameToStructureApplication.Dictionary;
using System.IO;
using NameToStructureApplication.Help;
    ...
    public partial class frmMain_PepsiLite : Form
    {
        ...
        private void EnableMenuItems(string _userrole)
        {
            try
            {
                ...
                retrieveDictStripMenuItem1.Enabled = true;
                lookUpToolStripMenuItem.Enabled = true;

                if (_userrole.Trim().ToUpper() == "PROJECT MANAGER")
                {
                    XMLToolStripMenuItem.Enabled = true;
                    userAccountsToolStripMenuItem.Enabled = true;                   
                    taskToolStripMenuItem.Enabled = true;
                    phaseToolStripMenuItem.Enabled = true;

                    insertDictToolStripMenuItem.Enabled = true;
                    deleteDictToolStripMenuItem1.Enabled = true;
                    updateDictToolStripMenuItem1.Enabled = true;
                }
                else if (_userrole.Trim().ToUpper() == "TEAM LEAD")
                {
                    XMLToolStripMenuItem.Enabled = true;
                    userAccountsToolStripMenuItem.Enabled = false;
                    taskToolStripMenuItem.Enabled = true;
                }
                else if (_userrole.Trim().ToUpper() == "REVIEWER")
                {
                    XMLToolStripMenuItem.Enabled = true;
                    taskToolStripMenuItem.Enabled = true;
                    userAccountsToolStripMenuItem.Enabled = false;
            ...
            try
            {
                ...
                if (objLogin.ShowDialog() != DialogResult.OK)
                {
                    if (objLogin.SubmitClick)
                    {
                        ...
                        statStripLbl_User.Text = Generic.Generic_PepsiLite.UserRole + ": " + Generic.Generic_PepsiLite.UserName;
        ...
        private void updateToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                Dictionary.frmUpdateDict objUpdateDict = new Dictionary.frmUpdateDict();
        ...
        private void insertDictToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                Dictionary.frmAddToDict objfrmDict = new Dictionary.frmAddToDict();
        ...
        private void retrieveDictStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                Dictionary.frmRetrieveDict objRetDict = new Dictionary.frmRetrieveDict();
        ...
        private void insertToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                TaskManagement.frmInsertPhaseDetails objInsertPDtls = new TaskManagement.frmInsertPhaseDetails();
                objInsertPDtls.ShowDialog();
            }
            catch (Exception ex)
            {
                PepsiLiteErrorHandling.WriteErrorLog(ex.ToString());
        ...
        private void lookUpToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                frmLookUpDict objLookDict = new frmLookUpDict();
                objLookDict.ShowDialog();
                //objLookDict.MdiParent = this;
                //objLookDict.Dock = DockStyle.Right;
                //objLookDict.Show();
            }
            catch (Exception ex)
            {
        ...
        private void userGuideToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                string strFileName = AppDomain.CurrentDomain.BaseDirectory + "PepsiLiteUserGuide.pdf";
                if (File.Exists(strFileName))
                {
                    FormCollection frmColl = Application.OpenForms;
                    Help.frmUserGuide objusrGuide = null;
                    bool blFrmOpen = false;
                    foreach (Form frm in frmColl)
                    {
                        if (frm.Name.ToUpper() == "FRMUSERGUIDE")
                        {
                            objusrGuide = (frmUserGuide)frm;
                            blFrmOpen = true;
                            objusrGuide.Show();
                            objusrGuide.WindowState = FormWindowState.Maximized;
                        }
                    }
                    if (!blFrmOpen)
                    {
                        objusrGuide = new Help.frmUserGuide();
                        objusrGuide.FileName = strFileName;
                        objusrGuide.MdiParent = this;
                        objusrGuide.Show();
                    }                    
                }
                else
                {
                    MessageBox.Show("No UserGuide found");
                }
            }
            catch (Exception ex)
            {
        ...
        private void menuStrip_PS_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
